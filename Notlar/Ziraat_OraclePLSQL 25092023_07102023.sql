...
--soru1
--Tüm satýþlar içinde son gün yapýlan satýþlarý bulunuz.
--Tablolar: Sales_Orders

Select *
from Sales_Orders
Where orderdate = (SELECT MAX(orderdate) FROM Sales_Orders)
Order By orderid desc;

--soru2
--En yüksek sayýda sipariþi olan müþterinin sipariþlerini listeleyiniz
--Tablolar: Sales_Orders
...
--soru3
--1 Mayýs 2008’ de veya daha sonra sipariþ almayan çalýþanlarý BULAN bir sorgu yazýnýz.
--Tablolar: HR_Employees ve Sales_Orders
/

SELECT
he.empid, he.firstname, he.lastname
FROM hr_employees he
LEFT JOIN sales_orders so ON so.empid = he.empid
AND so.orderdate >= TO_DATE('2008-05-01', 'YYYY-MM-DD')
WHERE so.orderdate IS NULL
GROUP BY he.empid, he.firstname, he.lastname;

--soru4
--Müþterilerin olduðu ancak çalýþanlarýn bulunmadýðý ülkeleri döndüren bir sorgu yazýn.
--Tablolar: Sales_Customers ve HR_Employees

SELECT DISTINCT sc.country
FROM Sales_Customers sc
LEFT JOIN hr_employees he ON sc.country = he.country
WHERE he.empid IS NULL
Order By country;

--soru5
--Her bir müþterinin en son sipariþ verdiði günün sipariþlerini listeleyiniz. 
--Tablolar : Sales_Orders
...
JOIN (
    ...
    FROM Sales_Orders
    GROUP BY custid
) subq ON so.custid = subq.custid AND so.orderdate = subq.latest_order_date;


--soru6
--Her bir müþterinin en son verdiði sipariþini listeleyiniz.
--Tablolar : Sales_Orders
...
--soru7
--2007 yýlýnda sipariþi olan fakat  2008 yýlýnda olmayan müþterileri bulunuz.
--Tablolar: Sales_Customers  and Sales_Orders
...
--soru8
--Müþterilerin sipariþleri ve ilerleyen tutar toplamlarýný ay bazýnda listeleyiniz.
--Tablolar : Sales_CustOrders 
...
WITH MonthlyOrders AS (
    ...
    FROM Sales_CustOrders
    GROUP BY custid, TO_CHAR(orderdate, 'YYYY-MM')
)
...
SELECT 
    ...
    OrderMonth,
    MonthlyTotal,
    SUM(MonthlyTotal) OVER (PARTITION BY custid ORDER BY OrderMonth) AS CumulativeTotal
FROM MonthlyOrders
...
s1.qty,
(Select Sum(s2.qty)
 From vw_Sales_CustOrders s2
 Where s1.custid=s2.custid
   and s2.orderdate <= s1.orderdate
) As RunQty

From vw_Sales_CustOrders s1
Order by s1.custid, s1.orderdate;
...
DECLARE
      ...
      PROCEDURE emp_oku (p_dept departments.department_id%type) IS
        BEGIN
            ...
            LOOP
                  dbms_output.put_line(I.employee_id ' ' || first_name || ' ' || I.last_name);
            END LOOP;
        End;
Begin
    emp_oku(60);
...
BEGIN
    SELECT department_name INTO wdeptname
    FROM DEPARTMENTS
    WHERE department_id = p_dept AND ROWNUM = 1;
...
END;
    ...
    BEGIN
        ...
        LOOP
            dbms_output.put_line(I.employee_id || ' ' || I.last_name);
        END LOOP;
        dbms_output.new_line;  -- Bir departmandaki çalýþanlar listelendikten sonra bir satýr boþluk ekleyelim.
    END;

BEGIN
    FOR J IN (SELECT department_id FROM DEPARTMENTS ORDER BY department_id) 
    LOOP
