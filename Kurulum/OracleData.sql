...
JOIN hr_employees he ON so.empid = he.empid
WHERE he.lastname LIKE 'D%'
...
LEFT JOIN Sales_Orders so ON sc.custid = so.custid 
...
-- yukarýdaki sorgu sipariþi olmayan müþterileri verir
...
JOIN LatestOrders la ON so.custid = la.custid 
where so.orderdate = la.MaxOrderDate
ORDER BY so.custid;
...
--join ile
Select * from Sales_OrderDetails sod
join Production_Products pp on sod.productid = pp.productid
WHERE
    sod.qty = 90
...
Select * from Sales_OrderDetails sod
where sod.productid in (Select pp.productid from Production_Products pp where qty = 90);

-- ornek 9
-- SPAIN'de sipariþi olan müþterilerin
--custid, companyname, country bilgilerini getiiriniz.
...
sc.country
from Sales_Customers sc
JOIN Sales_Orders so on sc.custid = so.custid
where sc.country = 'Spain';
...
-- Sales Customer tablosundaki tekrar eden verileri bul
Select
      Rowid,
      custid      
From Sales_Customers
...
-- -- Sales Customer tablosundaki tekrar eden verileri sil
...
-- Yukarýdkai uygulamayý geliþtirelim
...
Declare
    ...
    Type t_contact_list IS TABLE OF contact INDEX BY PLS_INTEGER; -- Contact dizisi

    rehber t_contact_list;

Begin
    rehber(1).kisi.ad:= 'Ali';
    rehber(1).kisi.soyad:= 'TOPACIK';
    rehber(1).telefon:= '0555 998 88 76';

    FOR i IN 1..rehber.COUNT LOOP
        dbms_output.put_line('Adi    : ' || rehber(i).kisi.ad);
        dbms_output.put_line('Soyadi : ' || rehber(i).kisi.soyad);
        dbms_output.put_line('Telefon: ' || rehber(i).telefon);
        dbms_output.put_line('---------');
    END LOOP;
End;
/
...
Open c_emp; -- Ýmleci aç

LOOP
    FETCH c_emp INTO r_emp; -- Ýmleçten bir satýr al ve r_emp kaydýna ata
    
    EXIT WHEN c_emp%NOTFOUND; -- Eðer imlec?te daha okunacak kayýt kalmamýþsa döngüden çýk
    
    isim_goster(r_emp.Employee_id); -- Çalýþanýn mevcut soyadýný ekrana yazdýr
    isim_guncelle(r_emp.Employee_id); -- Çalýþanýn soyadýný güncelle ve ekrana yazdýr
END LOOP;

CLOSE c_emp; -- Ýmleci kapat
...
Begin
    ...
    dbms_output.put_line('Employee_id= ' || 100 || 'Simdiki Maas = ' || 
    w_maas || 'Zamlý Yeni Maas=' || w_zamli_maas);
End;
...
CREATE OR REPLACE PROCEDURE MerhabaDunya IS
    ...
    BEGIN
                dbms_output.put_line('MerhabaDunya');
    END;
